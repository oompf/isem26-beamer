\documentclass[notheorems]{beamer}

% Packages, common definitions, etc.
\usepackage{csquotes}
\usepackage{graphicx}
\usepackage{xcolor}
\usepackage{comment}

% TikZ
\usepackage{tikz}
\usetikzlibrary{positioning} % Relative positioning
\usetikzlibrary{lindenmayersystems}

\usepackage{amsthm}
\setbeamertemplate{theorems}[numbered] % to number

\theoremstyle{definition}
\newtheorem{definition}{Definition}[section]
\newtheorem{example}[definition]{Example}
\theoremstyle{plain}
\newtheorem{theorem}[definition]{Theorem}
\newtheorem{lemma}[definition]{Lemma}
\newtheorem{corollary}[definition]{Corollary}
\theoremstyle{remark}
\newtheorem*{remark}{Remark}


\newcommand{\N}{\mathbb{N}} % natural numbers
\newcommand{\Z}{\mathbb{Z}} % integers
\newcommand{\Q}{\mathbb{Q}} % rational numbers
\newcommand{\K}{\mathbb{K}} % field
\newcommand{\R}{\mathbb{R}} % reals
\newcommand{\C}{\mathbb{C}} % complex numbers
\newcommand{\1}{\mathds{1}} % indicator
\newcommand{\A}{\mathcal{A}}
\newcommand{\G}{\mathcal{G}}
\newcommand{\F}{\mathcal{F}}
\newcommand{\X}{\mathcal{X}}
\newcommand{\T}{\mathcal{T}}
\newcommand{\Zer}{\mathscr{Z}}
\newcommand{\m}{\mathfrak{m}}
\newcommand{\frakB}{\mathfrak{b}}
\newcommand{\M}{\mathscr{M}}
\newcommand{\Hol}{\mathcal{H}}

% Added by Paul
\newcommand{\dwt}{$\Delta$-Y-Transform} % Delta-Wye-Transform
\newcommand{\abs}[1]{\left\vert #1 \right\vert} % Absolute value
\newcommand{\dom}[1]{\mathrm{dom}\left( #1 \right)} % Domain
\newcommand{\E}{\mathcal{E}} % Energy

\title{Energy Forms on Fractals}
\subtitle{The Case of the Sierpinksi Gasket}

\author % (optional, for multiple authors)
{Youssef Hakiki\inst{1} \and Marcel Kleinfeller\inst{2} \and Paul Rahlf\inst{3} \and \\[3mm] {\small Supervisor: Uta Freiberg\inst{4}}}

\institute[VFU] % (optional)
{
  \inst{1}%
  Cadi Ayyad University
  \and
  \inst{2}%
  Technical University of Darmstadt
  \and
  \inst{3}%
  Kiel University
  \and
  \inst{4}%
  Chemnitz University of Technology
}

\setbeamertemplate{footline}[page number]{} % Show page numbers
\setbeamertemplate{navigation symbols}{} % Suppress navigation symbols


\date % (optional)
{ISem26, 16-22 July 2023}
%\titlegraphic{\includegraphics[height=.4\textheight]{images/differential_equations_on_fractals.jpg}} % Add graphic to title page

\begin{document}
\begin{frame}
    \titlepage
\end{frame}

%\begin{frame}
%    \tableofcontents
%\end{frame}

\begin{frame}{Reference}
  \begin{columns}[c]
    \column{0.5\textwidth}
    \begin{figure}
        \centering
        \includegraphics[width=10em]{images/book.pdf}
        \caption{Book cover}
    \end{figure}
    \column{0.5\textwidth}
    \begin{thebibliography}{10}
      \bibitem{strichartz06}
      \alert{Robert S. Strichartz}
      \newblock  {Differential Equations on Fractals}
      \newblock {\em Princeton University Press (2006)}.
    \end{thebibliography}
\end{columns}
\end{frame}

\include{part1}
\include{part2}
\include{part3}
\end{document}